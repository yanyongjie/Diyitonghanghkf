...
import com.example.mytong.utils.SystemUtil;
...
public class HttpMessage {
    private static volatile HttpMessage httpMessage;

    public  HttpMessage(){}

    private OkHttpClient okhttpClient;

    public static   HttpMessage getInethttpMessage(){
        if (httpMessage==null){
    ...
    public ApiServer getServer() {
        Retrofit build = new Retrofit.Builder().baseUrl(Location.BaseUrl)
    ...
    private OkHttpClient getOkhttpClient() {
        //缓存文件定义
        Cache cache = new Cache(new File(MyApp.myApp.getCacheDir(), "Cache"), 1024 * 1024 * 10);
        //日志过滤器
